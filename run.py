...
from utils.utils import GeoData, Utilities
...
class Runner:
    ...
    def scrape_prices_and_details(
        self,
        scrape_prodej_byty: bool | None = True,
        scrape_all: bool | None = True,
    ) -> pd.DataFrame:
        ...
        if scrape_prodej_byty:
            logger.info("Running scraper for Prodeje - Byty.")
            ...
            self.utils.safe_save_csv(
                df_data_prodej_byty, f"data_{date_to_save}"
            )

        # scrape basically all
        if scrape_all:
            logger.info("Running scraper for All")
            data_all = self.scraper.scrape_all_with_filter(
        ...
        if scrape_prodej_byty:
            ...
            if len(df_missing) > 0:
                ...
                logger.info("Finished scraping missing estate details")
            else:
                logger.info("No need to scrape missing estate details")
        ...
        if scrape_all:
            ...
            if len(df_missing) > 0:
                self.scraper.scrape_specific_estates(df_missing, full_datetime)
                logger.info("Finished scraping missing estate details for ALL")
            else:
                logger.info("No need to scrape missing estate details for ALL")
    ...
    def input_all_estates_to_db(self):
        ...
        logger.info("Starting processing JSONs to estate_detail table.")
        ...
        if len(df_new) > 0:
            ...
            logger.info("Insert into estate_detail DONE.")
        else:
            logger.info("No need to insert into estate_detail. DONE.")
    ...
    def input_all_prices_to_db(self):
        ...
        logger.info("Starting processing CSVs to price_history table.")
        ...
        if len(df_new) > 0:
            ...
            logger.info("Inserting into price_history DONE.")
        else:
            logger.info("No need to insert into price_history. DONE.")
    ...
    def run_complete_scraping(
        ...
        logger.info("STARTING complete process of data gathering.")
        ...
        logger.info("FINISHING complete process of data gathering.")
