...
class Utilities:
    ...
    def safe_save_csv(self, df: pd.DataFrame, filename: str):
        ...
        if os.path.exists(file_path):
            logger.info(
                f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}: Name {filename} in {file_path} already exists. Saving as {filename}_SAFE"
            )
            filename += "_SAFE"
            file_path = f"{self.cf.project_path}/{self.cf.data_folder}/{self.cf.scraped_prices_folder}/{filename}.csv"
    ...
    def compare_codes_to_existing_jsons(
        self, codes_not_found: list[str]
    ) -> list[str]:
        ...
        for file_name in files:
            ...
            codes_not_found = [
                x for x in codes_not_found if x not in codes_from_json
            ]
    ...
    def prepare_estate_detail_jsons_to_df(self) -> pd.DataFrame:
        ...
        for file_name in tqdm(files):
            ...
            try:
                with open(
                    f"{self.cf.project_path}/{self.cf.data_folder}/{self.cf.geo_locations_folder}/geodata_{file_name}"
                ) as file2:
    ...
    def prepare_price_history_csv_to_df(self) -> pd.DataFrame:
        ...
        for file_name in tqdm(files):
            ...
            try:
                ...
                logger.error(
                    f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}: There was an error in file {file_name}"
                )
                continue

        df = pd.concat(dfs, ignore_index=True)
        df.rename(
            columns={"code": "estate_id", "timestamp": "crawled_at"},
            inplace=True,
        )
...
class GeoData:
    ...
    def get_location(self, lat: float, lon: float) -> str:
        try:
            geolocator = Nominatim(
                timeout=20, user_agent="JZ_Sreality"
            )  # Pomohlo změnit jméno, proti "Error 403" !!
    ...
    def parse_location(self, location: str) -> dict:
        try:
            ...
            if len(items) >= 7 and (
                items[-5].strip().startswith("okres")
                or items[-5].strip().startswith("obvod")
            ):
            ...
            if len(items) >= 6 and (
                items[-4].strip().startswith("okres")
                or items[-4].strip().startswith("obvod")
            ):
                return {
    ...
    def assign_location_to_df(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["locality_to_parse"] = df.progress_apply(
            lambda row: self.get_location(
                row["locality_gps_lat"], row["locality_gps_lon"]
            ),
            axis=1,
        )

        df[["locality", "city", "district", "region"]] = df[
            "locality_to_parse"
        ].apply(lambda x: pd.Series(self.parse_location(x)))
    ...
    def enrich_jsons_with_geodata(self, list_of_jsons: None) -> pd.DataFrame:
        ...
        files = (
            os.listdir(source_folder) if not list_of_jsons else list_of_jsons
        )
