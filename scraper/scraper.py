...
class SrealityScraper:
    ...
    def scrape_all_with_filter(
        self,
        timestamp: str,
        category_main_cb: int | None = None,  # 1 = Byt
        category_type_cb: int | None = None,  # 1 = Prodej
        category_sub_cb: int | None = None,  # 2 = 1+kk
        per_page: int | None = 999,  # 999 is max to show on single page
    ) -> pd.DataFrame:

        url_base = "https://www.sreality.cz/api/cs/v2/estates?"
        url_count = "https://www.sreality.cz/api/cs/v2/estates/count"
        url_count_prodej_byty = "https://www.sreality.cz/api/cs/v2/estates/count?category_main_cb=1&category_type_cb=1"
        ...
        with requests.Session() as session:
            result = session.request(
                method="GET", url=url_count, headers=headers
            ).json()
        logger.info(
            f"There are {result['result_size']} number of ALL estates (Byt, Dům, Pronájem, Prodej)"
        )
        ...
        with requests.Session() as session:
            result = session.request(
                method="GET", url=url_count_prodej_byty, headers=headers
            ).json()
        logger.info(
            f"There are {result['result_size']} number of Prodej-Byty estates."
        )
    ...
    def scrape_specific_estates(
        ...
        for code in tqdm(codes):
            ...
            try:
                ...
                d["broker_company"] = str(
                    r["_embedded"]["seller"]["_embedded"]["premise"]["name"]
                )
                #############
                d["furnished"] = int(r["recommendations_data"]["furnished"])
                d["locality_gps_lat"] = float(
                    r["recommendations_data"]["locality_gps_lat"]
                )
                d["locality_gps_lon"] = float(
                    r["recommendations_data"]["locality_gps_lon"]
                )
                d["object_type"] = int(r["recommendations_data"]["object_type"])
                d["parking_lots"] = int(
                    r["recommendations_data"]["parking_lots"]
                )
                d["locality_street_id"] = int(
                    r["recommendations_data"]["locality_street_id"]
                )
                d["locality_district_id"] = int(
                    r["recommendations_data"]["locality_district_id"]
                )
                d["locality_ward_id"] = int(
                    r["recommendations_data"]["locality_ward_id"]
                )
                d["locality_region_id"] = int(
                    r["recommendations_data"]["locality_region_id"]
                )
                d["locality_quarter_id"] = int(
                    r["recommendations_data"]["locality_quarter_id"]
                )
                d["locality_municipality_id"] = int(
                    r["recommendations_data"]["locality_municipality_id"]
                )
                d["locality_country_id"] = int(
                    r["recommendations_data"]["locality_country_id"]
                )
                ...
                d["building_type"] = int(
                    r["recommendations_data"]["building_type"]
                )
                ...
                d["building_condition"] = int(
                    r["recommendations_data"]["building_condition"]
                )
                d["garage"] = int(r["recommendations_data"]["garage"])
                d["room_count_cb"] = int(
                    r["recommendations_data"]["room_count_cb"]
                )
                d["energy_efficiency_rating_cb"] = int(
                    r["recommendations_data"]["energy_efficiency_rating_cb"]
                )
                #############
                for item in r["items"]:
